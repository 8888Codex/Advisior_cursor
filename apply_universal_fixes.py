"""
Script para aplicar Fixes Universais em todos os 18 clones em legends.py
Baseado em gaps_consolidados.md
"""
...
def apply_universal_fixes(file_path):
    ...
    # Cada prompt tem formato: NOME_PROMPT = """..."""
    ...
    def process_prompt(match):
        ...
        # 2. ADICIONAR "## FRAMEWORK NAMING PROTOCOL" após "### Técnicas e Métodos"
        # Encontra a seção "### Técnicas e Métodos" e adiciona o protocol depois dela
        if "### Técnicas e Métodos" in prompt_content:
            # Encontrar o fim da seção "Técnicas e Métodos" (próximo "##" ou fim)
            tecnicas_pattern = r'(### Técnicas e Métodos\n.*?)(\n## )'
            
            if re.search(tecnicas_pattern, prompt_content, re.DOTALL):
                prompt_content = re.sub(
                    tecnicas_pattern,
                    r'\1\n' + FRAMEWORK_NAMING_PROTOCOL.strip() + r'\n\2',
                    prompt_content,
                    flags=re.DOTALL,
                    count=1
                )
        ...
        if re.search(comm_pattern, prompt_content, re.DOTALL):
            prompt_content = re.sub(
                comm_pattern,
    ...
    print(f"✅ Framework Naming Protocol: {count_naming} prompts")
    print(f"✅ Callbacks Icônicos: {count_callbacks} prompts")
    print(f"✅ Protocolo de Recusa: {count_limitacoes} prompts")
    
    return True

if __name__ == "__main__":
    file_path = "python_backend/prompts/legends.py"
    apply_universal_fixes(file_path)
